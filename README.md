# Agent Skills

A collection of reusable skills for development workflows. Installable via the [`skills` CLI](https://github.com/vercel-labs/skills).

## Planning & Design

- **bc-grill-me** — Get relentlessly interviewed about a plan or design until every branch of the decision tree is resolved. _Adapted from [mattpocock/skills](https://github.com/mattpocock/skills)._

  ```
  npx skills@latest add ben-carmichael/skills/bc-grill-me
  ```

- **bc-issue-to-plan** — Create a concrete implementation plan from a GitHub issue, exploring the codebase to identify files and changes needed.

  ```
  npx skills@latest add ben-carmichael/skills/bc-issue-to-plan
  ```

- **bc-prd-to-issues** — Break a PRD into independently-grabbable GitHub issues using tracer-bullet vertical slices. _Adapted from [mattpocock/skills](https://github.com/mattpocock/skills)._

  ```
  npx skills@latest add ben-carmichael/skills/bc-prd-to-issues
  ```

- **bc-write-prd** — Create a PRD through user interview, codebase exploration, and module design, then submit as a GitHub issue. _Adapted from [mattpocock/skills](https://github.com/mattpocock/skills)._

  ```
  npx skills@latest add ben-carmichael/skills/bc-write-prd
  ```

## Development

- **bc-todo** — Create a structured TODO.md with Bugs, Features, and Chores sections.

  ```
  npx skills@latest add ben-carmichael/skills/bc-todo
  ```

## Writing

- **bc-write-case-study** — Generate a polished case study blog post from a code repository.

  ```
  npx skills@latest add ben-carmichael/skills/bc-write-case-study
  ```
